...
def validate_inputs():
    ...
    # Check that first_name is not blank, set error text if blank
    if len(first_name.get()) == 0:
        Label(main_window, text="Required", bg=main_window_bg_color, fg="red").grid(column=2, row=2, sticky=W)
        input_check = 1

    # Check that last_name is not blank, set error text if blank
    ...
    if len(amount_hired.get()) == 0:
        ...
        if int(amount_hired.get()) <= 0 or int(amount_hired.get()) > 500:
            Label(main_window, text="Between 0-500", bg=main_window_bg_color, fg="red").grid(column=2, row=5, sticky=W)
            input_check = 1
...
def setup_elements():
    ...
    img_frame4.grid(column=1, row=7, sticky=EW, pady=5)
...
first_name.grid(column=1, row=2)
first_name.config(insertbackground="white")
last_name = Entry(main_window, width=23, bg="#979BBA", fg="white")
last_name.grid(column=1, row=3)
last_name.config(insertbackground="white")
item_hired = Entry(main_window, width=23, bg="#979BBA", fg="white")
item_hired.grid(column=1, row=4)
